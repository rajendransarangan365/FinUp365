...
router.patch('/settings/:userId', async (req, res) => {
    ...
    try {
        ...
        // Validate reminderHoursBefore (should be positive number)
        if (reminderHoursBefore !== undefined) {
            const hours = Number(reminderHoursBefore);
            if (isNaN(hours) || hours < 0 || hours > 48) {
                return res.status(400).json({ error: "Invalid reminder hours (must be between 0-48)" });
            }
            user.reminderHoursBefore = hours;
        }
